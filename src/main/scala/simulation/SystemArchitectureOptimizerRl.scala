...
object SystemArchitectureOptimizerRl extends App with Logger with StreamingDimensionCalculator with OutputPortCalculator {
...
//      println("📥 Loading ML prediction model...")
    ...
    val architectureOptimizer = new ArchitectureOptimizerRl(
  ...
  private def generateLogFileName(config: SimulationConfig): String = {
    if(config.retentionRatio1.isDefined && config.retentionRatio2.isDefined && config.retentionRatio3.isDefined)
      s"/result_rl_${config.layerName}_mult:${config.totalNumberOfMultipliers}" +
