...
import common.{Dataflow, FilePaths, OutputPortCalculator}

import java.io.File
import scala.collection.mutable.ArrayBuffer
import scala.util.{Failure, Success, Try}

object SystemArchitectureOptimizerBo extends App with Logger with StreamingDimensionCalculator with OutputPortCalculator {
...
//      println("📥 Loading ML prediction model...")
    ...
    val architectureOptimizer = new ArchitectureOptimizerBo(
  ...
  private def generateLogFileName(config: SimulationConfig): String = {
    if(config.retentionRatio1.isDefined && config.retentionRatio2.isDefined && config.retentionRatio3.isDefined)
      s"/result_bo_${config.layerName}_mult:${config.totalNumberOfMultipliers}" +
